...
class PassRequest: UIViewController, UITextFieldDelegate
{
    ...
    func customInit()
    {
        ...
        lbl.text = "Введите пароль аудита"
        ...
        btnOk = redButton()
        btnOk.setTitle("Скачать", for: .normal)
        ...
        btnOk.click =
            {
                if tf.text != nil && tf.text == self.currentShablon.password!
                {
                    self.dismiss(animated: false, completion:
                        {
                            self.downloadAudit(fbId: self.currentShablon.fbId!)
                        })
